...
from utils.camera_utils_nerfstudio import get_interpolated_poses
...
def generate_path(
    ...
    elif path_type == "interpolated":
        new_poses = get_interpolated_poses(
            pose_recenter[start_frame], pose_recenter[end_frame], n_frames
        )
...
def create_videos(base_dir, input_dir, out_name, num_frames=480):
    ...
    depth_file_1 = os.path.join(input_dir, "depth", f"{idx_to_str(0)}.png")
    depth_frame_1 = load_img(depth_file_1)
    depth_file_2 = os.path.join(input_dir, "depth", f"{idx_to_str(num_frames-1)}.png")
    depth_frame_2 = load_img(depth_file_2)
    shape = depth_frame_1.shape
    p = 3

    depth_values = np.concatenate([depth_frame_1.flatten(), depth_frame_2.flatten()])

    distance_limits = np.percentile(depth_values, [p, 100 - p])
    ...
    for k in ["depth", "normal", "color"]:
        ...
        if k == "color":
            ...
            file0 = os.path.join(input_dir, "depth", f"{idx_to_str(0)}.{file_ext}")
        ...
        with media.VideoWriter(
            ...
            for idx in tqdm(range(num_frames)):
                ...
                if k == "color":
                    ...
                    img_file = os.path.join(
                        input_dir, "depth", f"{idx_to_str(idx)}.{file_ext}"
                    )

                elif k == "normal":
                    img_file = os.path.join(
                        input_dir, "normal", f"{idx_to_str(idx)}.{file_ext}"
                    )
