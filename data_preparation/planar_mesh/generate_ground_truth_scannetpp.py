...
def loadClassMap(scannetpp_folder):
    ...
    with open(
        scannetpp_folder.parent / "metadata" / "semantic_classes.txt"
    ) as info_file:
        classes = [line.rstrip() for line in info_file]
    ...
    for class_name in classes:
        for word in planar_words:
            if word in class_name.split():
                classMap[class_name] = word
                classLabelMap[class_name] = class_to_id[word]

        if class_name not in classMap:
            classMap[class_name] = "otherprop"
            classLabelMap[class_name] = class_to_id[class_name]

    classMap[""] = "unannotated"
    classLabelMap[""] = -100
    classLabelMap["unannotated"] = -100
...
def process_scene(scene_id, scannetpp_folder, output_folder):

    filename = scannetpp_folder / scene_id / "scans" / "segments_anno.json"
    data = json.load(open(filename, "r"))
    aggregation = np.array(data["segGroups"])

    filename = scannetpp_folder / scene_id / "scans" / "mesh_aligned_0.05_semantic.ply"

    plydata = PlyData.read(filename)
    vertices = plydata["vertex"]
    points = np.stack([vertices["x"], vertices["y"], vertices["z"]], axis=1)
    faces = np.array(plydata["face"]["vertex_indices"])
    labels = np.array(vertices["label"])

    labels = np.ascontiguousarray(labels)

    filename = scannetpp_folder / scene_id / "scans" / "segments.json"
    ...
    classMap, classLabelMap = loadClassMap(scannetpp_folder=scannetpp_folder)
    ...
    allXYZ = np.ascontiguousarray(allXYZ)
    ...
    for groupIndex, group in tqdm(enumerate(groupSegments), total=len(groupSegments)):
        ...
        allSegmentIndices = group
        segmentIndices = allSegmentIndices.copy()

        XYZ = allXYZ[segmentIndices]
        ...
        if minNumPlanes == 1 and maxNumPlanes == 1 and numRealPlanes > 1:
            ...
            pointIndices = np.concatenate(
                [
                    indices
                    for plane, indices in zip(groupPlanes, groupPlanePointIndices)
                ],
                axis=0,
            )
    ...
    for index, group in enumerate(planeGroups):
        ...
        groupInfo = [[(index, label)] for _ in range(len(groupPlanes))]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scannetpp", type=Path, required=True, help="Path to ScanNet++ scans"
    )
    parser.add_argument(
    ...
    scene_ids = [
        scene.name for scene in args.scannetpp.iterdir() if scene.name in args.scene_ids
    ]
    ...
    for index, scene_id in enumerate(tqdm(scene_ids)):
        ...
        num_planes = process_scene(
            scene_id, scannetpp_folder=args.scannetpp, output_folder=args.output
        )
